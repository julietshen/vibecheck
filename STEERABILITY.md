...
| **cope-b-a4b** | Highest aligned accuracy (F1 ~0.936 self-harm, ~0.885 sexual, precision ~1.0 on most self-harm policies, 100% clean output); fully policy-adherent and the only model to strongly adopt the inverted target (neg 0%→84%) | Largest cope model (27B total / 4B active MoE, ~54 GB in bf16), most costly to serve; very long policies can lower F1; sensitive to endpoint shape; verdict only, no reasoning or rationale |
